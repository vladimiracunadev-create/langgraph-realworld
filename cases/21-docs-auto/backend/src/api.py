...
app = FastAPI(title="Caso 21 - Documentacion Automatica")
...
class DocsIn(BaseModel):
    thread_id: str = Field(default="docs-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)
    repo_id: str = Field(default="DOC-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)


def _build_initial_state(repo_id: str) -> dict:
    return {
        "repo_id": repo_id,
        "repo": {},
        "artefactos": {},
...
def _build_snapshot(out: dict, repo_id: str) -> dict:
    return {
        "repo_id": out.get("repo_id", repo_id),
        "repo": out.get("repo", {}),
        "artefactos": out.get("artefactos", {}),
        "outline": out.get("outline", []),
        "secciones": out.get("secciones", []),
        "issues": out.get("issues", []),
        "iter_revision": out.get("iter_revision", 0),
        "score_global": out.get("score_global", 0),
        "coherencia": out.get("coherencia", {}),
        "documento_md": out.get("documento_md", ""),
        "diff": out.get("diff", {}),
...
@app.post("/api/run")
def run(payload: DocsIn):
    logger.info(f"Iniciando docs: thread_id={payload.thread_id} repo_id={payload.repo_id}")
    start_time = time.time()
    try:
        graph = get_graph()
        cfg = {"configurable": {"thread_id": payload.thread_id}, "recursion_limit": 50}
        out = graph.invoke(_build_initial_state(payload.repo_id), config=cfg)
        snapshot = _build_snapshot(out if isinstance(out, dict) else {}, payload.repo_id)
        duration = round(time.time() - start_time, 3)
        logger.info(
            f"Docs completadas en {duration}s: repo={snapshot.get('repo',{}).get('nombre')} "
...
@app.get("/api/stream")
def stream(
    thread_id: str = Query(default="docs-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
    repo_id: str = Query(default="DOC-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
):
    logger.info(f"Iniciando stream: thread_id={thread_id} repo_id={repo_id}")
    graph = get_graph()
    ...
    def gen():
        try:
            for event in graph.stream(
                _build_initial_state(repo_id), config=cfg, stream_mode="values"
            ):
                values = event if isinstance(event, dict) else {}
                snapshot = _build_snapshot(values, repo_id)
