...
app = FastAPI(title="Caso 22 - Backoffice Automatizacion")
...
class BackofficeIn(BaseModel):
    thread_id: str = Field(default="bo-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)
    solicitud_id: str = Field(default="SOL-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)
...
def _build_snapshot(out: dict, solicitud_id: str) -> dict:
    return {
        ...
        "solicitud": out.get("solicitud", {}),
        "operacion": out.get("operacion", {}),
        "solicitante": out.get("solicitante", {}),
        "permisos_ok": out.get("permisos_ok", False),
        "motivo_rechazo": out.get("motivo_rechazo", ""),
        "datos_completos": out.get("datos_completos", False),
        "campos_faltantes": out.get("campos_faltantes", []),
        "iter_completitud": out.get("iter_completitud", 0),
        "resultado_ejecucion": out.get("resultado_ejecucion", {}),
        "estado_final": out.get("estado_final", ""),
        "confirmacion": out.get("confirmacion", ""),
        "log_auditoria": out.get("log_auditoria", []),
        "hash_auditoria": out.get("hash_auditoria", ""),
...
@app.post("/api/run")
def run(payload: BackofficeIn):
    logger.info(f"Iniciando backoffice: thread_id={payload.thread_id} solicitud_id={payload.solicitud_id}")
    ...
    try:
        ...
        logger.info(
            f"Backoffice completado en {duration}s: solicitud={snapshot.get('solicitud',{}).get('id')} "
...
@app.get("/api/stream")
def stream(
    thread_id: str = Query(default="bo-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
    solicitud_id: str = Query(default="SOL-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
):
