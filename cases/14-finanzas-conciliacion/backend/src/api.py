...
app = FastAPI(title="Caso 14 - Finanzas: Conciliación")
...
class ConciliacionIn(BaseModel):
    thread_id: str = Field(default="conc-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)
    scenario_id: str = Field(default="SCN-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)


def _build_initial_state(scenario_id: str) -> dict:
    return {
        ...
        "outliers": [],
        "ajustes_propuestos": [],
        "escalaciones_auditoria": [],
        "partidas_en_transito": [],
        "metricas": {},
        "reporte_cuadre": "",
        "resumen": "",
        "events": [],
...
def _build_snapshot(out: dict, scenario_id: str) -> dict:
    return {
        "scenario_id": out.get("scenario_id", scenario_id),
        "periodo": out.get("periodo", ""),
        "descripcion_escenario": out.get("descripcion_escenario", ""),
        "total_bancarias": len(out.get("normalizadas_banco", [])),
        "total_contables": len(out.get("normalizadas_contables", [])),
        "matches": out.get("matches", []),
        "unmatched_banco": out.get("unmatched_banco", []),
        "unmatched_contables": out.get("unmatched_contables", []),
        "outliers": out.get("outliers", []),
        "ajustes_propuestos": out.get("ajustes_propuestos", []),
        "escalaciones_auditoria": out.get("escalaciones_auditoria", []),
        "partidas_en_transito": out.get("partidas_en_transito", []),
        "metricas": out.get("metricas", {}),
        "reporte_cuadre": out.get("reporte_cuadre", ""),
...
@app.post("/api/run")
def run(payload: ConciliacionIn):
    logger.info(f"Iniciando conciliación: thread_id={payload.thread_id} scenario_id={payload.scenario_id}")
    start_time = time.time()
    try:
        graph = get_graph()
        cfg = {"configurable": {"thread_id": payload.thread_id}, "recursion_limit": 50}
        out = graph.invoke(_build_initial_state(payload.scenario_id), config=cfg)
        snapshot = _build_snapshot(out if isinstance(out, dict) else {}, payload.scenario_id)
        duration = round(time.time() - start_time, 3)
        logger.info(
            f"Conciliación completada en {duration}s: periodo={snapshot.get('periodo')} "
            f"riesgo={snapshot.get('metricas', {}).get('riesgo')}"
        )
...
@app.get("/api/stream")
def stream(
    thread_id: str = Query(default="conc-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
    scenario_id: str = Query(default="SCN-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
):
    logger.info(f"Iniciando stream: thread_id={thread_id} scenario_id={scenario_id}")
    graph = get_graph()
    ...
    def gen():
        try:
            for event in graph.stream(
                _build_initial_state(scenario_id), config=cfg, stream_mode="values"
            ):
                values = event if isinstance(event, dict) else {}
                snapshot = _build_snapshot(values, scenario_id)
