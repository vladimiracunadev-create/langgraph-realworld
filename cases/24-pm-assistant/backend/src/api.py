...
app = FastAPI(title="Caso 24 - Asistente de Product Manager")
...
class PMIn(BaseModel):
    thread_id: str = Field(default="pm-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)
    iniciativa_id: str = Field(default="I-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN)


def _build_initial_state(iniciativa_id: str) -> dict:
    return {
        "iniciativa_id": iniciativa_id,
        "iniciativa": {},
        "preguntas_clarificadoras": [],
        "respuestas_clarificadoras": [],
        "epica": {},
        "historias": [],
        "estimaciones": {},
        "backlog_priorizado": [],
...
def _build_snapshot(out: dict, iniciativa_id: str) -> dict:
    return {
        "iniciativa_id": out.get("iniciativa_id", iniciativa_id),
        "iniciativa": out.get("iniciativa", {}),
        "preguntas_clarificadoras": out.get("preguntas_clarificadoras", []),
        "respuestas_clarificadoras": out.get("respuestas_clarificadoras", []),
        "epica": out.get("epica", {}),
        "historias": out.get("historias", []),
        "estimaciones": out.get("estimaciones", {}),
        "backlog_priorizado": out.get("backlog_priorizado", []),
        "tickets_creados": out.get("tickets_creados", []),
        "sprint_asignado": out.get("sprint_asignado", {}),
        "progreso_sprint": out.get("progreso_sprint", {}),
        "impedimentos": out.get("impedimentos", []),
        "estado_sprint": out.get("estado_sprint", ""),
        "reporte": out.get("reporte", ""),
        "retrospectiva": out.get("retrospectiva", {}),
...
@app.post("/api/run")
def run(payload: PMIn):
    logger.info(f"Iniciando PM: thread_id={payload.thread_id} iniciativa_id={payload.iniciativa_id}")
    start_time = time.time()
    try:
        graph = get_graph()
        cfg = {"configurable": {"thread_id": payload.thread_id}, "recursion_limit": 50}
        out = graph.invoke(_build_initial_state(payload.iniciativa_id), config=cfg)
        snapshot = _build_snapshot(out if isinstance(out, dict) else {}, payload.iniciativa_id)
        duration = round(time.time() - start_time, 3)
        logger.info(
            f"PM completado en {duration}s: iniciativa={snapshot.get('iniciativa',{}).get('id')} "
...
@app.get("/api/stream")
def stream(
    thread_id: str = Query(default="pm-demo-1", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
    iniciativa_id: str = Query(default="I-001", min_length=1, max_length=64, pattern=SAFE_ID_PATTERN),
):
    logger.info(f"Iniciando stream: thread_id={thread_id} iniciativa_id={iniciativa_id}")
    graph = get_graph()
    ...
    def gen():
        try:
            for event in graph.stream(
                _build_initial_state(iniciativa_id), config=cfg, stream_mode="values"
            ):
                values = event if isinstance(event, dict) else {}
                snapshot = _build_snapshot(values, iniciativa_id)
