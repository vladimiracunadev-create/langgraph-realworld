# Caso 02: Soporte Técnico de Empleados / SRE Helpdesk

**Estado:** `OPERATIVO` (v3.7.0)
...
6. **`request_approval`**: (*HITL - Human In The Loop*) Detiene el flujo de máquina temporalmente simulando requerir aprobación de un SRE Lead si la categoría toca infraestructura.
7. **`execute_runbook`**: Ejecución en consola estéril simulada y logueada línea por línea.
8. **`validate_resolution`** y **`draft_response`**: Checkeo de log vs ticket de origen, redactando el correo a cliente cerrando el incidente.
