"""
auth.py — Middleware de autenticación y guardrails para Caso 01.
...
El modo DEMO nunca se rompe: si USE_OAUTH2=false (default) y DEMO_AUTH_TOKEN
está vacío, todas las peticiones pasan sin restricción, igual que antes.
"""
...
# ---------------------------------------------------------------------------
# Helpers de configuración (leen env en cada request para testabilidad)
# ---------------------------------------------------------------------------
...
def use_oauth2() -> bool:
    """Activa el modo OAuth2/OIDC. False por defecto (backward-compatible)."""
...
# ---------------------------------------------------------------------------
# Validación JWT (solo cuando USE_OAUTH2=true)
# ---------------------------------------------------------------------------

def _validate_jwt(token: str) -> None:
    """
    Valida un Bearer JWT contra el JWKS del proveedor OIDC configurado.
    Lanza ValueError si el token es inválido o las variables no están configuradas.
    """
    ...
    try:
        ...
        raise ValueError("python-jose no instalado — agrega python-jose[cryptography] a requirements") from exc
...
async def auth_middleware(
    ...
    """
    Middleware unificado de auth + rate-limit.
    Devuelve None si la petición debe continuar, o un JSONResponse de error.
    """
